...
cat("\n Final hands:\n")
final_scores <- score_game(game_result)
print(final_scores)

# Step 6: Show final rankings
cat("\n Player Rankings:\n")
ranking_df <- tibble::tibble(
  Player = names(final_scores),
  Cards_Left = as.integer(final_scores),
  Position = paste0(dplyr::row_number(final_scores),
                    dplyr::case_when(
                      dplyr::row_number(final_scores) == 1 ~ "st",
                      dplyr::row_number(final_scores) == 2 ~ "nd",
                      dplyr::row_number(final_scores) == 3 ~ "rd",
                      TRUE ~ "th"
                    ))
)
print(ranking_df)

# Step 7: Show game log
cat("\n🕹️ Game log:\n")
print(game_result$game_log)
